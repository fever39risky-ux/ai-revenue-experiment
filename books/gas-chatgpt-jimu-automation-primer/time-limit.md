...
:::message
この「続きから再開」の仕組みを、スプレッドシートの `=AI()` 関数と組み合わせて組み込み済みにしたものが[スプレッドシート =AI() 関数・GAS拡張版（$3）](https://feverish50.gumroad.com/l/ymotl)です。数百行の一括処理（実行前に円換算の費用見積もり、途中で止まっても続きから再開）と、月の予算上限が入っています。
:::
