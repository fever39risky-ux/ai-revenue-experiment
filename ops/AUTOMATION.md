...
- `.github/workflows/gumroad-publish.yml` — manual (`workflow_dispatch`) only; same trigger pattern as Etsy publish. `gumroad_publish.mjs` calls Gumroad's official REST API v2 directly (no CLI, first- or third-party) to upload the file, create, and publish the listing defined in `marketing/gumroad_listing_config.json`. Idempotent (`status/gumroad_listing.json`). **Needs `GUMROAD_ACCESS_TOKEN`** — see `ops/GUMROAD_API_SETUP.md`. Simpler grant than Etsy's OAuth (a personal access token from Settings → Advanced, no app registration).
...
- `scripts/gumroad_publish.mjs` — runs in Actions; calls Gumroad's REST API v2 directly to upload the file, create, and publish the listing from `marketing/gumroad_listing_config.json`.
...
4. `GUMROAD_ACCESS_TOKEN` — single personal access token generated from Settings → Advanced on the seller's own Gumroad account (see `ops/GUMROAD_API_SETUP.md`). No CLI install needed. Account is already KYC'd/bank-linked; this is the only remaining step for a parallel listing.
